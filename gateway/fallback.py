...
from .tracing import layer_trace
...
class Fallback:
    ...
    async def generate(
        ...
        # L0: cache
        async with layer_trace(parent_run_id=request_id, layer="L0_cache",
                               provider="redis", model_id=model.id,
                               prompt_chars=len(prompt)) as t:
        ...
        if prov:
            async with layer_trace(parent_run_id=request_id, layer="L1_free",
                                   provider=prov.name, model_id=model.id,
        ...
        if "local_cpu" in model.providers and model.id in GGUF_MAP:
            async with layer_trace(parent_run_id=request_id, layer="L2_cpu",
                                   provider="llama_cpp_ampere", model_id=model.id,
                                   prompt_chars=len(prompt)):
                try:
        ...
        if "spot" in model.providers or "local_gpu" in model.providers:
            ...
            if inst:
                async with layer_trace(parent_run_id=request_id, layer="L4_spot",
                                       provider=f"vast:{inst.provider}", model_id=model.id,
                                       prompt_chars=len(prompt)):
                    try:
        ...
        for m in matrix.all():
            if "local_cpu" in m.providers and m.id in GGUF_MAP:
                async with layer_trace(parent_run_id=request_id, layer="L5_degraded",
                                       provider="llama_cpp_ampere", model_id=m.id,
                                       prompt_chars=len(prompt)):
                    try:
